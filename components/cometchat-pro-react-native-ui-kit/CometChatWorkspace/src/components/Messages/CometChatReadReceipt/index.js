...
const CometChatReadReceipt = (props) => {
  ...
  const [isDeliveryReceiptsEnabled, setIsDeliveryReceiptsEnabled] = useState(true);
  useEffect(() => {
    checkRestrictions();
  });
  const checkRestrictions = async () => {
    const isEnabled = await context.FeatureRestriction.isDeliveryReceiptsEnabled();
  ...
  const timestamp = new Date(
    props.message.sentAt ? props.message.sentAt * 1000 : props.message._composedAt
  ...
  //   const minutes = time[1] && Number(time[1]?.split(' ')[0]);
  //   console.log({ timestamp, time, minutes });
