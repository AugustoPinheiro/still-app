        ...
        this.MessageListManager.fetchPreviousMessages()
          .then((messageList) => {
            if (messageList.length === 0) {
              this.decoratorMessage = 'Nenhuma mensagem encontrada';
            }
          ...
          .catch((error) => {
            ...
            logger('[CometChatMessageList] getMessages fetchPrevious error', error);
          });
      })
      .catch((error) => {
        this.decoratorMessage = 'Error';
        logger('[CometChatMessageList] getMessages getLoggedInUser error', error);
      });
  };
    ...
    try {
      ...
      if (
        ...
        if (Object.prototype.hasOwnProperty.call(message, 'metadata')) {
          this.props.actionGenerated(actions.CUSTOM_MESSAGE_RECEIVED, [message]);
          ...
          this.props.actionGenerated(actions.CUSTOM_MESSAGE_RECEIVED, [newMessage]);
  ...
  groupUpdated = (key, message, group, options) => {
    try {
      if (
        ...
        this.props.actionGenerated(actions.GROUP_UPDATED, message, key, group, options);
      }
    } catch (error) {
      logger(error);
    }
      ...
      switch (message.type) {
        ...
        case CometChat.MESSAGE_TYPE.IMAGE: {
          if (message.data?.attachments) {
            const parsedMessage = {
              ...message,
              data: { ...message?.data, url: message?.data?.attachments?.[0]?.url },
            };
            component = (
    ...
    if (Object.prototype.hasOwnProperty.call(message, 'deletedAt')) {
      ...
      switch (message.type) {
        ...
        case CometChat.MESSAGE_TYPE.TEXT:
          ...
          break;
        case CometChat.MESSAGE_TYPE.IMAGE: {
          if (!message.data.url && message.data?.attachments) {
            const parsedMessage = {
              ...message,
              data: { ...message?.data, url: message?.data?.attachments?.[0]?.url },
            };
    ...
    const newMsgPopUp = (
      <View style={styles.newMessagePopupContainer}>
        ...
        >
          ...
          <Icon name="arrow-down" style={{ marginLeft: 5 }} size={15} color="#000" />
        </TouchableOpacity>
      </View>
    );
