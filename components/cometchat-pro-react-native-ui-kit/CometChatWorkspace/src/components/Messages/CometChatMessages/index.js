...
import { CometChatContextProvider, CometChatContext } from '../../../utils/CometChatContext';
...
class CometChatMessages extends React.PureComponent {
  ...
  checkRestrictions = async () => {
    const context = this.contextProviderRef.state;
    const isGroupActionMessagesEnabled =
      await context.FeatureRestriction.isGroupActionMessagesEnabled();
    const isCallActionMessagesEnabled =
      await context.FeatureRestriction.isCallActionMessagesEnabled();
    const isOneOnOneChatEnabled = await context.FeatureRestriction.isOneOnOneChatEnabled();
    const isGroupChatEnabled = await context.FeatureRestriction.isGroupChatEnabled();
    const isHideDeletedMessagesEnabled =
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (params.type === 'user' && prevParams.item.uid !== params.item.uid) {
      ...
      // this.setUserDetails()
    } else if (params.type === 'group' && prevParams.item.guid !== params.item.guid) {
      this.setState({
        messageList: [],
        scrollToBottom: true,
        messageToBeEdited: '',
    ...
    this.setState(
      ...
      () => {
        this.props.route?.params?.actionGenerated('groupDeleted', group) ||
          (this.props.actionGenerated && this.props.actionGenerated('groupDeleted', group));
  ...
  messageSent = (message) => {
    ...
    const messageKey = messageList.findIndex((m) => m._id === message._id);
    if (messageKey > -1) {
  ...
  memberUnbanned = (members) => {
    ...
    const messageList = [...this.state.messageList];
    const filteredMembers = _.uniqBy(members, 'id');
  ...
  deleteMessage = (message) => {
    ...
    CometChat.deleteMessage(messageId)
      .then((deletedMessage) => {
        ...
        params.actionGenerated('updateThreadMessage', [deletedMessage], 'delete');
  ...
  render() {
    ...
    let messageComposer = (
      <CometChatMessageComposer
        ...
        theme={this.theme}
        item={params.type === CometChat.RECEIVER_TYPE.USER ? this.state.user : this.state.item}
        type={params.type}
          ...
          <SafeAreaView style={style.chatWrapperStyle}>
            ...
            {this.state.userDetailVisible ? (
              <CometChatUserDetails
                ...
                theme={this.theme}
                item={
                  params.type === CometChat.RECEIVER_TYPE.USER ? this.state.user : this.state.item
                }
                type={params.type}
                actionGenerated={this.actionHandler}
              />
