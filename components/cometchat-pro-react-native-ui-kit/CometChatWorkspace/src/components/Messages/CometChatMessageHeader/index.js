...
class CometChatMessageHeader extends React.Component {
  ...
  checkRestrictions = async () => {
    const isGroupVideoCallEnabled =
      await this.context.FeatureRestriction.isGroupVideoCallEnabled();
    const isOneOnOneAudioCallEnabled =
    ...
    const isOneOnOneVideoCallEnabled =
  ...
  setStatusForUser = () => {
    try {
      ...
      if (
        ...
        const messageTimestamp = new Date(this.props.item.lastActiveAt * 1000);
