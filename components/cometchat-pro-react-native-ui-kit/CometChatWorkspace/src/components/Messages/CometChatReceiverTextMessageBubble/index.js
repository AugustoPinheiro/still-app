...
const CometChatReceiverTextMessageBubble = (props) => {
  ...
  const checkRestrictions = async () => {
    const isLinkPreviewEnabled = context.FeatureRestriction.isLinkPreviewEnabled();
            ...
            >
              <View style={style.messagePreviewWrapperStyle}>
                <Image
                  ...
                  source={{
                    uri: linkObject.image ? linkObject.image : linkObject.favicon,
                  }}
                  contentFit="contain"
                />
                <View
                ...
                <TouchableOpacity
                  ...
                  onPress={async () =>
                    await Linking.canOpenURL(linkObject.url).then((supported) => {
                      if (!supported) return;

                      Linking.openURL(linkObject.url);
                    })
                  }
                >
                  <Text
        ...
        <View>
          {props.message.receiverType === CometChat.RECEIVER_TYPE.GROUP ? (
            <View style={style.senderNameStyle}>
              <Text style={{ color: props.theme.color.helpText }}>{message.sender.name}</Text>
            </View>
          ) : null}
