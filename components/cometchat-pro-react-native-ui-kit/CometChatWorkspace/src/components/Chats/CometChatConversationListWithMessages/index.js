...
class CometChatConversationListWithMessages extends React.Component {
  ...
  componentDidMount() {
    ...
    new CometChatManager()
      ...
      .catch((error) => {
        logger('[CometChatConversationListWithMessages] getLoggedInUser error', error);
      });
  }
  ...
  itemClicked = (item, type) => {
    this.setState({ item: { ...item }, type, viewDetailScreen: false }, () => {
      this.props.navigation.navigate(enums.NAVIGATION_CONSTANTS.COMET_CHAT_MESSAGES, {
        theme: this.theme,
        item: { ...item },
        tab: this.state.tab,
        type,
      ...
      CometChatManager.unblockUsers(usersList)
        .then((response) => {
          if (response) {
            ...
            this.dropDownAlertRef?.showMessage('success', 'Failed to unblocked user');
          }
        })
        .catch((error) => {
