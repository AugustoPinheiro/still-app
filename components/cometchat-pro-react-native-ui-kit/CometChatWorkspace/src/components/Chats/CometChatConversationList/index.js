...
class CometChatConversationList extends React.Component {
  ...
  checkRestrictions = async () => {
    const isMessagesSoundEnabled = await this.context.FeatureRestriction.isMessagesSoundEnabled();
    ...
    try {
      ...
      if (
        ...
        const groupKey = conversationList.findIndex(
          (member) => member.conversationWith.guid === this.props.groupToDelete.guid
        );
        if (groupKey > -1) {
          conversationList.splice(groupKey, 1);
          this.setState({ conversationList });
          if (conversationList.length === 0) {
            this.decoratorMessage = 'Nenhum chat encontrado';
          }
        }
      }
    } catch (error) {
    ...
    try {
      switch (key) {
        ...
        case enums.GROUP_MEMBER_ADDED:
          if (this.loggedInUser.uid !== actionBy.uid) this.updateGroupMemberAdded(message, options);
          break;
