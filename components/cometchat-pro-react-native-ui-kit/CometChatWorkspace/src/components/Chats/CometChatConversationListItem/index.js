...
import { CometChatUserPresence, CometChatBadgeCount, CometChatAvatar } from '../../Shared';
...
class CometChatConversationListItem extends React.Component {
  ...
  checkAndMarkAsDelivered = async () => {
    ...
    if (
      lastMessage &&
      lastMessage.sender.uid != this.props.loggedInUser.uid &&
  ...
  checkRestrictions = async () => {
    const isUnreadCountEnabled = await this.context.FeatureRestriction.isUnreadCountEnabled();
  ...
  getLastMessage = () => {
    try {
      if (Object.prototype.hasOwnProperty.call(this.props, conversation) === false) {
        return false;
      }

      if (Object.prototype.hasOwnProperty.call(this.props.conversation, lastMessage) === false) {
        return false;
      }
  ...
  getLastMessageTimestamp = () => {
    try {
      ...
      if (
        Object.prototype.hasOwnProperty.call(this.props.conversation.lastMessage, sentAt) === false
      ) {
        return false;
      }
      ...
      if (diffTimestamp < 24 * 60 * 60 * 1000) {
        timestamp = messageTimestamp.toLocaleTimeString('pt-BR', {
          hour: 'numeric',
          minute: 'numeric',
          hour12: false,
        });
        ...
        //   const time = timestamp.split(':'); // convert to array
        ...
        //   timeValue += hours; // get AM/PM
        //   timestamp = timeValue;
        // }
      } else if (diffTimestamp < 48 * 60 * 60 * 1000) {
        timestamp = 'Ontem';
      } else if (diffTimestamp < 7 * 24 * 60 * 60 * 1000) {
        timestamp = messageTimestamp.toLocaleDateString('pt-BR', {
          year: '2-digit',
          month: '2-digit',
          day: '2-digit',
        });
        ...
        >
          ...
          >
            ...
            >
              ...
              <View style={styles.itemLastMsgStyle}>{lastMessageTimeStamp}</View>
            </View>
            <View
              style={{
                flexDirection: 'row',
