  ...
  banMember = (memberToBan) => {
    try {
      ...
      CometChat.banGroupMember(guid, memberToBan.uid)
        .then((response) => {
          if (response) {
            this.dropDownAlertRef?.showMessage('success', 'Group member banned');
            this.props.actionGenerated(actions.BAN_GROUP_MEMBERS, memberToBan);
          } else {
            this.dropDownAlertRef?.showMessage('error', 'Failed to ban group member');
          }
        })
        .catch((error) => {
  ...
  kickMember = (memberToKick) => {
    try {
      ...
      CometChat.kickGroupMember(guid, memberToKick.uid)
        .then((response) => {
          if (response) {
            this.dropDownAlertRef?.showMessage('success', 'Group member kicked');
            this.props.actionGenerated(actions.REMOVE_GROUP_PARTICIPANTS, memberToKick);
          } else {
            this.dropDownAlertRef?.showMessage('error', 'Failed to kick group member');
          }
        })
        .catch((error) => {
          const errorCode = error?.message || 'ERROR';
  ...
  changeScope = (member, scope) => {
    try {
      ...
      CometChat.updateGroupMemberScope(guid, member.uid, scope)
        .then((response) => {
          if (response) {
            this.dropDownAlertRef?.showMessage('success', 'Group member scope changed');
            const updatedMember = { ...member, scope };
            this.props.actionGenerated(actions.UPDATE_GROUP_PARTICIPANTS, updatedMember);
          } else {
            this.dropDownAlertRef?.showMessage('error', 'Failed to change scope of group member');
          }
        })
        .catch((error) => {
          const errorCode = error?.message || 'ERROR';
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        >
          <TouchableOpacity onPress={() => this.props.close()} style={style.container}>
            <BottomSheet
              ref={this.sheetRef}
