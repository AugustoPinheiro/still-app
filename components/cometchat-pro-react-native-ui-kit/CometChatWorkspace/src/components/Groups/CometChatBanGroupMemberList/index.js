...
export default class CometChatBanGroupMemberList extends React.Component {
  ...
  unbanMember = (memberToUnBan) => {
    ...
    CometChat.unbanGroupMember(guid, memberToUnBan.uid)
      .then((response) => {
        if (response) {
          this.dropDownAlertRef?.showMessage('success', 'Group member unbanned');
          this.props.actionGenerated(actions.UNBAN_GROUP_MEMBERS, [memberToUnBan]);
        } else {
          this.dropDownAlertRef?.showMessage('error', 'Failed to unban group member ');
        }
      })
      .catch((error) => {
