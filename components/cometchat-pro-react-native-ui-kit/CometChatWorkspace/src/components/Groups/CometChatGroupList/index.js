...
class CometChatGroupList extends React.Component {
  ...
  componentDidMount() {
    try {
      this.navListener = this.props.navigation.addListener('focus', () => {
        this.decoratorMessage = 'Carregando...';
  ...
  checkRestrictions = async () => {
    ...
    const isGroupSearchEnabled = await context.FeatureRestriction.isGroupSearchEnabled();
    const isGroupCreationEnabled = await context.FeatureRestriction.isGroupCreationEnabled();
    const isJoinLeaveGroupsEnabled = await context.FeatureRestriction.isJoinLeaveGroupsEnabled();
    this.setState({
      restrictions: {
      ...
      CometChat.joinGroup(this.state.guid, this.state.groupType, passcode)
        .then((response) => {
          if (typeof response === 'object') {
            this.dropDownAlertModelRef?.showMessage('success', 'Group joined Successfully');
          } else {
            this.dropDownAlertModelRef?.showMessage('error', 'Failed to join group');
            return;
          }
          const groups = [...this.state.grouplist];
      ...
      if (group.type === CometChat.GROUP_TYPE.PUBLIC) {
        CometChat.joinGroup(group.guid, group.type, '')
          .then((response) => {
            const groups = [...this.state.grouplist];
            if (typeof response === 'object') {
              this.dropDownAlertRef?.showMessage('success', 'Group Joined Successfully');
            } else {
                  ...
                  >
                    <View style={styles.passwordScreenMainContainer}>
                      <View style={styles.passwordScreenInnerContainer}>
                        ...
                        <Text style={styles.passwordScreenHeader}>Password Required!</Text>
                        <View
                          style={[
