...
export function DetailsSchedule({ navigation, route: { params } }: DetailsScheduleProps) {
  ...
  // Carregar estado de "peguei todos os itens na mala" do storage
  React.useEffect(() => {
    async function loadPackedStatus() {
      try {
        const packedStatus = await AsyncStorage.getItem(`event_packed_${item?.id}`);
        setIsPacked(packedStatus === 'true');
      } catch (error) {
        console.error('Erro ao carregar status de mala:', error);
      }
    }
  ...
  // Função para alternar o status de "peguei todos os itens na mala"
  const handleTogglePacked = async () => {
    try {
      const newPackedStatus = !isPacked;
      setIsPacked(newPackedStatus);
      
      await AsyncStorage.setItem(`event_packed_${item?.id}`, newPackedStatus.toString());
      
      show({
        message: newPackedStatus 
          ? 'Itens marcados como na mala!' 
          : 'Status de mala desmarcado',
        type: 'success',
      });
    } catch (error) {
      console.error('Erro ao salvar status de mala:', error);
      show({
        ...
        <S.Content>
          ...
          {activeAccordion && (
            <S.MoreDetails>
              ...
              <S.Value>
                {format(dateWithTimezone(new Date(item.start_date)), 'dd/MM/yyyy')}
              </S.Value>
              <S.Label>Fim</S.Label>
              <S.Value>
                {format(dateWithTimezone(new Date(item?.end_date)), 'dd/MM/yyyy')}
              </S.Value> */}
              {/* {item?.tags.length && (
                <>
          ...
          {/* Botão para marcar "peguei todos os itens na mala" */}
          {(item?.looks?.length > 0 || item?.clothings?.length > 0) && (
            <S.PackedContainer>
              <TouchableOpacity onPress={handleTogglePacked} style={{ flexDirection: 'row', alignItems: 'center' }}>
                <S.PackedCheckbox isPacked={isPacked} />
                <S.PackedText>Coloquei todos os itens na mala</S.PackedText>
              </TouchableOpacity>
            </S.PackedContainer>
          )}
