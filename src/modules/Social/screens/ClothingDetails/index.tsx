...
export function ClothingDetails({ route: { params } }: ClothingDetailsProps) {
  ...
  const renderAttributes = React.useCallback(() => {
    ...
    return attributesFiltered.map((attribute) => {
      ...
      if (!hasAttribute)
        return <React.Fragment key={`no-content-${attribute.id}`}></React.Fragment>;

      switch (attribute.type) {
        case 'color':
          return (
            <S.ContentAtt key={`content-${attribute.id}`}>
              <S.Label>{attribute.title}</S.Label>
              <S.ColorContent>
                {attribute.attributes
                  .filter((item) => clothingAttributesIds?.includes(item.id))
                  .map((item) => (
                    <ColorTag key={`${item.id}`} color={item.icon} selected={true} />
                  ))}
              </S.ColorContent>
            </S.ContentAtt>
          );
  ...
  if (userData?.id === clothing?.owned_by_id) {
    navigation.dispatch(StackActions.pop(1));

    return navigation.dispatch(
      TabActions.jumpTo('Profile', { screen: 'ProfileClothingDetails', params: { id: itemId } })
    );
  }
    ...
    <S.Wrapper>
      <S.Container>
        <S.ImageContainer>
          ...
          <S.Image source={{ uri: clothing?.image }} contentFit="contain" cachePolicy="disk" />
        </S.ImageContainer>
