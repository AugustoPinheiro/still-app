...
export default class CometChatMessageComposer extends React.PureComponent {
  ...
  checkRestrictions = async () => {
    const isLiveReactionsEnabled = await this.context.FeatureRestriction.isLiveReactionsEnabled();
    const isTypingIndicatorsEnabled =
      await this.context.FeatureRestriction.isTypingIndicatorsEnabled();
    const isSmartRepliesEnabled = await this.context.FeatureRestriction.isSmartRepliesEnabled();
    this.setState({
      restrictions: {
  ...
  sendMediaMessage = (messageInput, messageType) => {
    try {
      ...
      CometChat.sendMessage(mediaMessage)
        ...
        .catch((error) => {
          const newMessageObj = { ...mediaMessage, error };
          const errorCode = error?.message || 'ERROR';
          this.props.actionGenerated(actions.ERROR_IN_SEND_MESSAGE, newMessageObj);
  ...
  sendTextMessage = () => {
    try {
      ...
      CometChat.sendMessage(textMessage)
        ...
        .catch((error) => {
          const newMessageObj = { ...textMessage, error };
          this.props.actionGenerated(actions.ERROR_IN_SEND_MESSAGE, newMessageObj);
          logger('Message sending failed with error:', error);
          const errorCode = error?.message || 'ERROR';
          this.props?.showMessage('error', errorCode);
  ...
  sendSticker = (stickerMessage) => {
    ...
    CometChat.sendCustomMessage(customMessage)
      ...
      .catch((error) => {
        const newMessageObj = { ...customMessage, error };
        this.props.actionGenerated(actions.ERROR_IN_SEND_MESSAGE, newMessageObj);
        const errorCode = error?.message || 'ERROR';
  ...
  render() {
    ...
    if (!this.state.messageInput.length && this.state.restrictions?.isLiveReactionsEnabled) {
      sendBtn = null;
    } else {
    ...
    if (disabled) {
      blockedPreview = (
        ...
        >
          ...
          >
            Para iniciar conversas, você precisa desbloquear este usuário
          </Text>
        </View>
      );
    }
      ...
      <View
        style={
          Platform.OS === 'android' && this.state.keyboardActivity
            ...
            : Platform.OS === 'ios' && this.state.keyboardActivity
            ? {
                paddingBottom: 102,
                elevation: 5,
                backgroundColor: '#fff',
