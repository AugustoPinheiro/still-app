...
import { View, Text, SafeAreaView, TouchableOpacity, Keyboard, Platform } from 'react-native';
...
class CometChatMessageThread extends React.PureComponent {
  ...
  checkIsHideDeletedEnabled = async () => {
    const hideDeletedEnabled = this.context.FeatureRestriction.isHideDeletedMessagesEnabled();
        ...
        case actions.MESSAGE_COMPOSED:
          {
            ...
            this.appendMessage(messages);
            this.props.actionGenerated(actions.THREAD_MESSAGE_COMPOSED, messages);
          }
          break;
      ...
      CometChat.deleteMessage(messageId)
        .then((deletedMessage) => {
          ...
          if (messageList.length - messageKey === 1 && !message.replyCount) {
            this.props.actionGenerated(actions.THREAD_MESSAGE_DELETED, [deletedMessage]);
          }
        })
        .catch((error) => {
          ...
          >
            <View style={[styles.headerWrapperStyle]}>
              <TouchableOpacity
                ...
                onPress={() => this.props.actionGenerated(actions.CLOSE_THREAD_CLICKED)}
              >
                {backIcon}
                <Text
                  style={{
              ...
              <View style={styles.fullFlex}>
                <CometChatMessageList
                  ...
                  parentMessageComponent={() => (
                    <>
                      <View style={styles.parentMessageStyle}>{parentMessage}</View>
                      <View
                        style={[
