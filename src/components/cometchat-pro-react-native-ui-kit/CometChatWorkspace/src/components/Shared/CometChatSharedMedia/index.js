...
export default class CometChatSharedMedia extends React.Component {
      ...
      .then((user) => {
        ...
        this.SharedMediaManager.fetchPreviousMessages()
          ...
          .catch((error) => {
            ...
            logger('[CometChatSharedMedia] getMessages fetchPrevious error', error);
          });
      })
      .catch((error) => {
        const errorCode = error?.message || 'ERROR';
        this.props?.showMessage('error', errorCode);
        logger('[CometChatSharedMedia] getMessages getLoggedInUser error', error);
      });
  };
  ...
  emptyListComponent = () => {
    return (
      <View style={styles.emptyComponentContainerStyle}>
        <Text style={styles.emptyComponentStyle}>{`No ${this.getActiveType()}`}</Text>
      </View>
    );
  };
  ...
  render() {
    ...
    const template = (message) => {
      ...
      if (messageType === CometChat.MESSAGE_TYPE.FILE && message.data.attachments) {
        return (
          <TouchableOpacity
            style={[styles.fileItemStyle, { backgroundColor: bgColor }]}
            onPress={async () =>
              await Linking.canOpenURL(message.data.attachments[0].url).then((supported) => {
                if (!supported) return;

                Linking.openURL(message.data.attachments[0].url);
              })
            }
          >
            ...
            <Text
              ...
              numberOfLines={1}
              style={[styles.fileStyle, { color: `${currentTheme.color.primary}` }]}
            >
      ...
      <View style={[styles.sectionStyle, {}]}>
        ...
        <View style={[styles.sectionContentStyle]}>
          <View style={styles.mediaBtnStyle}>
            <TouchableOpacity
              onPress={() => this.mediaClickHandler(CometChat.MESSAGE_TYPE.IMAGE)}
              style={[
                messageType === CometChat.MESSAGE_TYPE.IMAGE
            ...
            {messageType === CometChat.MESSAGE_TYPE.FILE ? <View style={styles.separator} /> : null}
            <TouchableOpacity
              onPress={() => this.mediaClickHandler(CometChat.MESSAGE_TYPE.VIDEO)}
              style={
            ...
            <TouchableOpacity
              onPress={() => this.mediaClickHandler(CometChat.MESSAGE_TYPE.FILE)}
              style={[
          ...
          <FlatList
            ...
            contentContainerStyle={messages?.length ? null : styles.mediaItemStyle}
