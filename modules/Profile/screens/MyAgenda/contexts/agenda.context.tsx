...
export const AgendaProvider = ({ children }: Props) => {
  ...
  function getLooksToday(data: string) {
    ...
    for (const schedule of schedules) {
      const startIsMoreThanEnd = new Date(schedule.start_date) > new Date(schedule.end_date);
      const hasToday = isWithinInterval(new Date(data), {
        start: new Date(sanitizeDate(startIsMoreThanEnd ? schedule.end_date : schedule.start_date)),
        end: new Date(sanitizeDate(startIsMoreThanEnd ? schedule.start_date : schedule.end_date)),
      });
